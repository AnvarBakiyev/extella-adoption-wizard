...
BRIDGE_VERSION = "3.85"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
        ...
        if self.path == "/x/expert":
                ...
                if _fid_pub and _flow_pub:
                    try:
                        ...
                        _restore = (
                            ...
                            '  try:\n'
                            '   _cur=api("/api/kv/get",{"key":"_mkt_automations","global":True}).get("value")\n'
                            ...
                            '  api("/api/kv/set",{"key":"_mkt_automations","value":json.dumps(_cat,ensure_ascii=False),"description":"automations catalog","global":True})\n'
            ...
            for cid, cq, ch in _core:
                _kw = {"data_sources": ["данн", "источник", "data", "source", "формат", "выгруз"],
                       ...
                       "success": ["успех", "критери", "метрик", "success", "metric"]}[cid]
