...
BRIDGE_VERSION = "4.25"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
def _step_gate(goal, step, result):
    """Трас-гейт на КАЖДЫЙ шаг: независимая скептическая проверка, что результат шага реально верен (не
    заглушка/ошибка/выдумка), ДО того как он войдёт в факты. Иначе ошибка накапливается по цепочке.
    Мягкая деградация: блокируем только при явном ok:false (не рубим на неоднозначном верификаторе)."""
    ag = qwen_agent()
    if not ag:
        return {"ok": True, "why": ""}
    v = _agent_json(ag, "Цель: " + str(goal)[:600] + "\nШаг: " + str(step)[:400] +
                    "\nРезультат шага: " + str(result)[:700] +
                    "\n\nПроверь СКЕПТИЧЕСКИ: результат корректен и действительно выполняет ЭТОТ шаг (не "
                    'заглушка, не сообщение об ошибке, не выдумка, не обрыв)? Верни JSON {"ok":true|false,"why":"<кратко>"}.')
    return {"ok": v.get("ok") is not False, "why": str(v.get("why", ""))[:160]}
...
def _goal_loop(goal, max_iters=3):
    ...
    for it in range(max(1, min(int(max_iters or 3), 6))):
        plan = _agent_json(ag, "Цель: " + goal + "\nСделано (накоплено):\n" + ftext() +
                           ...
                           '{"kind":"reason|build|need_human|done","step":"<что делаем / что спросить у человека / итог>","why":"<кратко>"}. '
                           ...
                           "преобразовать, дёрнуть API) — под него напишется и запустится код; reason — шаг решается "
                           "рассуждением/текстом без кода.")
        kind = str(plan.get("kind", "")).strip().lower()
        if kind == "compute":
            kind = "reason"
        step = str(plan.get("step", ""))[:400]
        if kind not in ("reason", "build", "need_human", "done") or not step:
        ...
        if kind == "done":
            ...
            if v.get("met") is True:
                ...
                return {"ok": True, "status": "done", "iters": it + 1, "journal": journal, "facts": facts}
        ...
        if kind == "build":
            ...
            if code:
                ...
                if err is None and res is not None:
                    rs = str(res)[:280]
                    g = _step_gate(goal, step, rs)   # трас-гейт на ШАГ: до принятия в факты
                    if not g["ok"]:
    ...
    return {"ok": True, "status": "max_iters", "iters": len(journal), "journal": journal, "facts": facts}
