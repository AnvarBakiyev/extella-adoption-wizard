...
BRIDGE_VERSION = "3.45"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
def _run_build(session_id, build_id):
    ...
    llm = {"api_key": CONFIG.get("llm_api_key", ""), "model": CONFIG.get("llm_model", "gpt-4o"),
           ...
           # keyless-кодоген идёт на ЭТОТ агент. Только Qwen клиента (config.agent_id) — НЕ Claude-дефолт agent_extella_default (жёг бы баланс).
           "agent_id": CONFIG.get("llm_agent_id") or CONFIG.get("agent_id", "")}
    ...
    def do_POST(self):
        ...
        if self.path == "/x/expert":
            ...
            if expert in ("wz_generate_blueprint", "wz_data_reality_check"):
                ...
                params.setdefault("agent_id", CONFIG.get("llm_agent_id") or CONFIG.get("agent_id", ""))  # Qwen клиента, НЕ Claude-дефолт
