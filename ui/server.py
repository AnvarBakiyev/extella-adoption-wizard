...
BRIDGE_VERSION = "4.96"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
def _web_junk_reason(res):
    ...
    доставка цветов — Гульжан, 20.07). Смысловой гейт `_stage_sanity` ловит это на СБОРКЕ и мягко
    помечает шаг «требует доводки», но у уже собранного каркаса клиента ничего не мешает прогону
    выдать «success · 60 000 000» поверх мусора. Чиним на НАШЕМ слое прогона: смотрим в готовый
...
# per-agent) и кому деплоим; эксперты/сессии/KV остаются общими (канон ТЗ). Создание keyless-Qwen =
# КОПИЯ базового «Qwen Extella» (провайдер alibaba, qwen3.7-max) — канон соблюдён.
...
def _agent_create_copy(name):
    ...
    # create может вернуть id, хотя фактически это Pro custom-agent без привязанного BYOK.
    # Такая карточка выглядит готовой, но первый же чат падает с pro_key_required. До регистрации
    # делаем короткий фактический запуск; нерабочую QA-копию сразу удаляем, не оставляя мусор.
    probe = api("/api/agent/run", {"agent_id": nid, "input": "Ответь одним словом: готов.",
                                    "run_timeout": 45, "store": False}, timeout=55)
    if not isinstance(probe, dict) or probe.get("status") == "error":
        ...
        if "pro_key_required" in raw or "provider API key" in raw:
            why = ("Платформа создала Pro-агента, но не разрешила ему работать без BYOK-ключа. "
                   "Выберите или привяжите уже работающего Qwen-агента; пустую карточку я удалил.")
        else:
...
def _agent_link(agent_id):
    """Привязать СУЩЕСТВУЮЩЕГО агента по id (round-trip): валидируем через agent/get, кладём в реестр."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/x/goal_loop":
            ...
            ok = isinstance(out, dict) and out.get("ok")
            # срез 2: память контура (концепты/правила) → в МОЗГ выбранного агента (per-agent), если выбран
            if ok and isinstance(out.get("memory"), dict):
                cur = _kv_read(CURAGENT_KV, "") or ""
                mem = out["memory"]
        ...
        if self.path == "/x/expert":
            ...
            if isinstance(upd, dict) and upd.get("id") == agent_id:
                ...
                # владельца — в rules того же агента; повторный deploy синхронизирует их диффом.
