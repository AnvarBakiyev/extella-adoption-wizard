...
BRIDGE_VERSION = "4.19"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
def _trust_check(task, sample, fields, runs=3):
    ...
    def do_POST(self):
        ...
        if self.path == "/x/trust_check":
            ...
            if not task or not sample or not fields:
                ...
                return
            out = _trust_check(task, sample[:20], [str(f)[:40] for f in fields][:12], runs)
            ok = isinstance(out, dict) and out.get("ok")
