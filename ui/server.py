...
BRIDGE_VERSION = "5.25"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/x/expert":
            ...
            _local_tgt = local_target() if expert in _local_file_experts else ""
            if expert in _local_file_experts and not _local_tgt:
            ...
            if expert in ("wz_generate_blueprint", "wz_data_reality_check"):
                ...
                res = run_expert(expert, params, glob=True, target=(_local_tgt or None))   # global + pin: код общий, файлы этого устройства
