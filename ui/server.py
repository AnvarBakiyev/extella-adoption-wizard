...
BRIDGE_VERSION = "3.76"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/x/expert":
            ...
            lang = _task_lang(task + " " + " ".join(allref))
            if lang == "ru":
            ...
            for _att in range(2):   # флап бэкенда Qwen отпускает за секунды → короткий ретрай
                res = run_expert("wz_auto_compose", {"task": full_task, "agent_id": qwen_agent(), "api_token": CONFIG.get("auth_token", "")},
                                 wait=200, glob=True)
                if isinstance(res, dict) and res.get("status") == "success":
                    break
                time.sleep(2)
