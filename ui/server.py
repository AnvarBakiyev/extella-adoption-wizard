...
BRIDGE_VERSION = "4.23"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
def _goal_loop(goal, max_iters=3):
    ...
    for it in range(max(1, min(int(max_iters or 3), 6))):
        plan = _agent_json(ag, "Цель: " + goal + "\nСделано (накоплено):\n" + ftext() +
                           ...
                           '{"kind":"compute|need_human|done","step":"<что делаем / что спросить у человека / итог>","why":"<кратко>"}. '
                           "done — если цель уже достигнута накопленным; need_human — если нужен человек "
                           "(доступ, решение, данные, которых нет); compute — если шаг можно выполнить рассуждением "
                           "или имеющимися данными.")
        kind = str(plan.get("kind", "")).strip().lower()
        step = str(plan.get("step", ""))[:400]
        if kind not in ("compute", "need_human", "done") or not step:
        ...
        # compute: выполнить шаг (LLM даёт конкретный результат) → в факты (накопление = память контура)
        ...
        journal.append({"iter": it + 1, "kind": "compute", "step": step, "result": result[:280]})
