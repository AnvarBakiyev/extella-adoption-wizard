...
BRIDGE_VERSION = "4.54"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
    ...
    def do_POST(self):
        ...
        if self.path == "/x/source_bind":
            ...
            s["source"] = {"kind": kind, "basename": basename, "source_key": skey,
                           ...
                           "schema": _source_schema(out)}   # отпечаток структуры для контроля дрифта
            s["updated_at"] = datetime.now(timezone.utc).isoformat()
            sp.write_text(json.dumps(s, ensure_ascii=False, indent=2), encoding="utf-8")
        ...
        if self.path == "/x/expert":
            ...
            if s.get("source"):
                ...
                if _drift.get("breaking"):
                    self._send({"status": "error", "code": "source_drift",
                                "message": "источник изменил структуру: исчезли колонки " +
                                ", ".join(_drift.get("removed", [])) + ". Проверьте источник или пересоберите процесс.",
                                "drift": _drift}, 409)
                    return
