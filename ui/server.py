...
BRIDGE_VERSION = "3.69"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
        ...
        if self.path == "/x/expert":
            ...
            if bpp.exists():
                try:
                    ...
                    bp = {}
            goal = str(bp.get("goal") or bp.get("summary") or "")[:600]
            ...
            try:
                ...
                self._send({"status": "error", "message": _scrub("генерация полей: " + str(e)[:150])})
                return
            clean = []
            used = set()
            ...
            if not clean:
                ...
                return
            manifest = {"fields": clean, "generated_at": datetime.now(timezone.utc).isoformat()}
