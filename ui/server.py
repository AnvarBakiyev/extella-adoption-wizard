...
BRIDGE_VERSION = "3.41"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
def run_expert(expert_name, params, wait=300, target=None, glob=False):
    ...
    if task_id:
        ...
        while time.time() - t0 < wait:
            ...
            if status not in ("pending", "running", "in_progress", "queued", ""):
...
def _build_one(expert_name, task, schema_hint, is_first, is_last, accept_input, llm):
        ...
        try:
            if llm.get("api_key"):
                ...
                rq = _u.Request(llm.get("api_base", "https://api.extella.ai").rstrip("/") + "/api/agent/run",
                                data=json.dumps({"agent_id": "agent_extella_default",
                                                 "input": sysmsg + "\n\n" + user + "\n\nВерни СТРОГО валидный JSON {\"description\":..,\"code\":..} без markdown.",
                                                 "run_timeout": 240, "store": True}).encode(),
            ...
            return False, None, "LLM: " + str(e)[:150]
        code = spec.get("code", "")
    ...
    return False, None, "3 попытки: " + str(why or "не прошли приёмку")[:150]
...
def _run_build(session_id, build_id):
    ...
    llm = {"api_key": CONFIG.get("llm_api_key", ""), "model": CONFIG.get("llm_model", "gpt-4o"),
           ...
           "api_token": CONFIG.get("auth_token", ""), "api_base": BASE}   # платформенная Qwen, если ключа нет
    ...
    try:
        ...
        r = run_expert("wz_build_plan", dict(session_id=session_id, namespace=ns, **tok, **llm), wait=900)   # Qwen-план тяжелее OpenAI
    ...
    def do_POST(self):
        ...
        if self.path == "/x/expert":
            ...
            if expert in ("wz_generate_blueprint", "wz_data_reality_check"):
                ...
                params.setdefault("agent_id", CONFIG.get("agent_id", "agent_extella_default"))
