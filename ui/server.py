...
BRIDGE_VERSION = "4.39"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
def _brain_read(agent_id, cap=150):
    """Ф3 кабинет-мозг: читаем МОЗГ выбранного агента — концепты (знания) и правила (per-agent скоуп).
    Умения (эксперты) — общий граф (канон ТЗ), поэтому здесь только счётчик, не редактируем тут."""
    cs, rs = [], []
...
def _agent_create_copy(name):
    ...
    cr = api("/api/agent/create", {
        ...
        "description": "Клиентский агент (копия Qwen Extella) — создан визардом",
        "instructions": base.get("instructions") or "",
        "tools": base.get("tools") or [], "model_parameters": base.get("model_parameters") or {}})
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path in ("/x/voice_learn", "/x/voice_try"):
            # Слой знание-и-голос: обучить агента голосу по примерам / примерить выученный голос
            if self._blocked_origin():
                self._send({"status": "error", "message": "forbidden origin"}, 403)
                return
            cur = _kv_read(CURAGENT_KV, "") or ""
