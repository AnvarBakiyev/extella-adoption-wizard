...
from wz_platform import CONFIG, BASE, HEADERS, _scrub, api, parse_expert_result, run_expert, qwen_agent, qwen_agents, local_target
...
from wz_local_experts import run_local_system_expert
...
BRIDGE_VERSION = "5.26"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
def _render_msg(template, name, count, total):
    ...
    if isinstance(template, str) and template.strip():
        ...
        # Default delivery must describe the result actually returned.  The old default always
        # printed a finance-shaped count/sum block, even for folder cleanup and text workflows.
        t = DEFAULT_MSG_TEMPLATE
...
def _process_action_lock(sid):
    """Один короткий mutation/resume UPC на сессию.

    Два быстрых клика раньше оба успевали прочитать ``repairing`` до checkpoint первого,
    запускали две стройки и второй worker видел чужой шаг как ``running``. Отдельный лок
    не держит файловый session-lock во время ``_start_build_job`` и потому не дедлочит его.
    """
    with _SESS_LOCKS_GUARD:
        lk = _PROCESS_ACTION_LOCKS.get(sid)
        if lk is None:
            lk = _PROCESS_ACTION_LOCKS[sid] = threading.Lock()
        return lk
...
def _llm_error_human(res):
    ...
    low = blob.lower()
    if _platform_auth_error(res):
        return _platform_auth_message()
...
def _platform_auth_error(res):
    """Отличить потерю платформенной сессии от отсутствующего агента и BYOK модели.

    API при остановленной/неавторизованной Extella отвечает ``user_id missing``. Раньше этот
    ответ терялся внутри KV/agent lookup, и UI делал опасный ложный вывод «агент не найден».
    Ошибки ключа провайдера сюда не относятся: для них остаётся отдельная подсказка о BYOK.
    """
    if not isinstance(res, dict):
        return False
    blob = json.dumps(res, ensure_ascii=False).casefold()
    if "user_id missing" in blob:
        return True
    if "api key" in blob or "apikey" in blob or "provider" in blob:
        return False
    try:
        code = int(res.get("http_code") or 0)
    except (TypeError, ValueError):
        code = 0
    return code == 401 and ("authentication required" in blob or "unauthorized" in blob)


def _platform_auth_message():
    return ("Extella на этом устройстве остановлена или потеряла авторизацию. Нажмите Start внизу "
            "окна и дождитесь Running, затем перезагрузите Wizard. Если уже показано Running — "
            "выйдите и снова войдите в Extella. Агент не удалён; создавать новый не нужно.")
...
def _kv_read_checked(key, default=None):
    """KV-read с сохранением транспортного статуса — нужен, чтобы empty != auth failure."""
    ...
    if isinstance(r, dict) and isinstance(r.get("value"), str):
        try:
            return True, json.loads(r["value"]), r
        except Exception:
...
def _kv_read(key, default=None):
    return _kv_read_checked(key, default)[1]
...
def _agents_state():
    ...
    reg_ok, reg, reg_raw = _kv_read_checked(AGENTS_KV, [])
    cur_ok, cur, cur_raw = _kv_read_checked(CURAGENT_KV, "")
    if not reg_ok or not cur_ok:
        raw = reg_raw if not reg_ok else cur_raw
        return {"agents": [], "current": "", "platform_ready": False,
                "auth_required": _platform_auth_error(raw),
                "why": (_llm_error_human(raw) or
                        "Нет связи с платформой Extella. Проверьте интернет и повторите через минуту.")}
    return {"agents": reg if isinstance(reg, list) else [], "current": cur or "",
            "platform_ready": True, "auth_required": False, "why": ""}
...
def _agent_create_copy(name):
    ...
    if _platform_auth_error(cr):
        return {"ok": False, "code": "platform_auth_required", "err": _platform_auth_message()}
...
def _start_build_job(sid):
    """Единый идемпотентный старт обычной и продолженной стройки.

    Проверка и запись ``session.building`` выполняются одним атомарным session update:
    двойной клик или две поверхности получают один build_id, а не два worker одного UPC.
    """
    build_id = "build_" + datetime.now(timezone.utc).strftime("%Y%m%d_%H%M") + "_" + uuid.uuid4().hex[:4]
    build_dir = RUNS_DIR / build_id
    build_dir.mkdir(parents=True, exist_ok=True)
    progress_path = build_dir / "build_progress.json"
    ...
    chosen = {"build_id": build_id, "existing": False}

    def mark(s):
        active = str(s.get("building") or "")
        if active and SAFE_ID.match(active):
            try:
            ...
            if active_status == "running":
                chosen.update({"build_id": active, "existing": True})
                return
...
class Handler(BaseHTTPRequestHandler):
        ...
        if path in ("/", "/wizard.html", "/index.html"):
            ...
            if not aid:
                ...
                self._send({"status": "success", "agent_id": aid, "ok": ok,
                            "auth_required": (not ok and _platform_auth_error(r)),
            ...
            for p in sorted(SESS_DIR.glob("wz_*.json"), reverse=True):
                ...
                if bpp.exists():
                    try:
                        ...
                        bp = None
                process_graph = {}
                process_path = SESS_DIR / (s.get("session_id", "") + "_process.json")
                ...
                resolved_recipients = _recipients(s)
                runtime_contract = project_runtime_contract(
                    process_graph, s, lb, bp or {}, recipients=resolved_recipients)
                out.append({
                    ...
                    "recipients": resolved_recipients,   # кабина «Настройка»: получатели результата (несколько каналов)
                    "message_template": s.get("message_template") or "",   # кабина «Шаблон сообщения»
                    "source": s.get("source") or None,
                    # Фактический I/O-контракт кабинета. Синтетический build_fixture не становится
                    # «ручным файлом», а доставка/финансовые поля не появляются без договора на них.
    ...
    def do_POST(self):
        ...
        if self.path == "/x/expert":
            ...
            # Эти эксперты читают/пишут ~/extella_wizard на устройстве, где открыт Wizard.
            # Без target при двух Listener'ах задача могла выполниться на другом Mac: платформа
            # возвращала success, но локальный bridge не видел созданный blueprint/spec/session.
            _local_file_experts = {"wz_session", "wz_generate_blueprint", "wz_project_spec", "wz_data_reality_check"}
            # wz_session — чистое локальное файловое хранилище и часть signed bundle моста.
            # Ему не нужна очередь Listener: иначе Stopped превращался в ложный user_id missing
            # ещё до первого вопроса. Остальные файловые эксперты по-прежнему пинним на этот Mac.
            _bridge_local_experts = {"wz_session"}
            _local_tgt = local_target() if (expert in _local_file_experts and
                                             expert not in _bridge_local_experts) else ""
            if expert in _local_file_experts and expert not in _bridge_local_experts and not _local_tgt:
                self._send({"status": "error", "code": "local_device_unknown",
                            "message": "Wizard не определил текущее устройство. Перезапустите Extella Listener и повторите действие."}, 503)
                return
            if expert == "wz_session":
                params.setdefault("base_dir", str(SESS_DIR))
            elif expert == "wz_project_spec":
                params.setdefault("base_dir", str(SESS_DIR))
                _sid_spec = str(params.get("session_id", ""))
                if SAFE_ID.match(_sid_spec or ""):
                    params.setdefault("output_path", str(SESS_DIR / (_sid_spec + "_spec.md")))
            elif expert == "wz_generate_blueprint":
                _sid_plan = str(params.get("session_id", ""))
                if SAFE_ID.match(_sid_plan or ""):
                    params.setdefault("session_path", str(SESS_DIR / (_sid_plan + ".json")))
                    params.setdefault("output_path", str(SESS_DIR / (_sid_plan + "_blueprint.json")))
            ...
            if expert in ("wz_generate_blueprint", "wz_data_reality_check"):
                ...
                _llm_result = run_llm_expert(expert, params, target=_local_tgt)
                ...
                res = (run_local_system_expert(expert, params) if expert in _bridge_local_experts
                       else run_expert(expert, params, glob=True, target=(_local_tgt or None)))
            ...
            if not SAFE_ID.match(sid or "") or not sp.exists() or not pp.exists():
                ...
                return
            action_lock = _process_action_lock(sid)
            if not action_lock.acquire(blocking=False):
                try:
                    active = str(json.loads(sp.read_text(encoding="utf-8")).get("building") or "")
                except Exception:
                    active = ""
                self._send({"status": "success", "action": action, "step_id": step_id,
                            "already_started": True, "build_id": active,
                            "message": "ремонт этого шага уже запускается"})
                return
            try:
                session = json.loads(sp.read_text(encoding="utf-8"))
                ...
                if active:
                    ...
                    if active_status == "running":
                        ...
                        self._send({"status": "success", "action": action, "step_id": step_id,
                                    "already_started": True, "build_id": active,
                                    "message": "стройка уже выполняет этот граф"})
                        return
            except Exception as exc:
                action_lock.release()
            ...
            if isinstance(g, dict) and g.get("value"):
                try:
                    ...
                    pass
            # Превью только с фактическими значениями последнего прогона. Фальшивые 128 позиций и
            # 26 млн ₸ раньше попадали даже в кабинет уборки Downloads.
            _last = (s.get("runs") or [])[-1] if (s.get("runs") or []) else {}
            preview = _render_msg(tpl, s.get("client_name") or sid,
                                  _last.get("total_count"), _last.get("total_sum"))
            ...
            for _att in range(2):   # устойчивость к флапу бэкенда Qwen (ретрай)
                res = run_local_system_expert(
                    "wz_auto_compose",
                    {"task": ctask, "agent_id": qwen_agent(), "api_token": CONFIG.get("auth_token", "")})
                if isinstance(res, dict) and res.get("status") == "success":
                    break
                time.sleep(2)
            ...
            for _att in range(2):   # флап бэкенда Qwen отпускает за секунды → короткий ретрай
                res = run_local_system_expert("wz_auto_compose", _cparams)
                if isinstance(res, dict) and res.get("status") == "success":
                    break
                time.sleep(2)
            if not isinstance(res, dict) or res.get("status") != "success":
