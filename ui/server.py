...
BRIDGE_VERSION = "4.78"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
def _report_spec_from_words(current, phrase, fields, agent_id=None):
    ...
    prompt = ("Ты проектируешь ВИД отчёта по просьбе владельца. Верни ТОЛЬКО JSON-спеку без пояснений:\n"
              ...
              "(«в ворде», «чтобы дописать», «редактируемый»); both — оба файла;\n"
    ...
    spec["format"] = _f if _f in ("pdf", "docx", "both") else "pdf"
    ...
    if spec.get("format") != (old.get("format") or "pdf"):
        changes.append("формат: " + {"pdf": "PDF", "docx": "Word (можно дорабатывать)",
                                     "both": "PDF и Word"}.get(spec["format"], spec["format"]))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/x/expert":
            ...
            _exp = "fmt_report_docx" if _fmt == "docx" else "fmt_report_pdf"
            _ext = ".docx" if _exp.endswith("docx") else ".pdf"
