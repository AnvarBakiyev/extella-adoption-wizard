...
BRIDGE_VERSION = "3.39"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
    ...
    for attempt in range(3):
        try:
            ...
            with _u.urlopen(rq, timeout=150) as r:
                spec = json.loads(json.loads(r.read().decode())["choices"][0]["message"]["content"])
        except Exception as e:
...
def _run_build(session_id, build_id):
    ...
    try:
        ...
        r = run_expert("wz_build_plan", dict(session_id=session_id, namespace=ns, **tok, **llm), wait=600)
