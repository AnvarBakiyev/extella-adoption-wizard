...
BRIDGE_VERSION = "4.33"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
def _sandbox_run(code, fn_name="step", arg=None):
    ...
    Так шаг контура = РЕАЛЬНО сгенерённый и исполненный эксперт. Возвращает (result, err)."""
    import ast as _ast
...
def _gen_step_code(goal, step, facts):
    """Контур пишет ЭКСПЕРТА под шаг: Qwen генерит `def step(facts)` — реальное вычисление результата."""
    ag = qwen_agent()
    if not ag:
        return None
...
def _step_gate(goal, step, result):
    ...
    обогащается структурированным знанием по ходу. Мягкая деградация: блок только при явном ok:false."""
    ag = qwen_agent()
    if not ag:
        return {"ok": True, "why": "", "concepts": [], "rules": []}
    ...
    return {"ok": v.get("ok") is not False, "why": str(v.get("why", ""))[:160], "concepts": cs, "rules": rs}


def _capability_search(need, hf_tag=None, limit=3):
    """Добыча способности: ищет ГОТОВУЮ способность (модель HF / репо GitHub) под шаг, который нельзя
    просто написать кодом с нуля (ML/3D-модель, спец-инструмент). READ-ONLY разведка кандидатов —
    сама установка это действие на устройстве и идёт через гейт человека (канон предложи-подтверди).
    hf_tag — канонический HF pipeline-тег (напр. depth-estimation): фильтр по тегу даёт ТОП-модели
    по загрузкам, что надёжнее подстрочного поиска по имени."""
    import urllib.parse as _up
    q = str(need)[:120].strip()
    cands = []
    if not q and not hf_tag:
        return cands

    def _get(url, params, headers=None):
        rq = urllib.request.Request(url + "?" + _up.urlencode(params),
                                    headers=dict(headers or {}, **{"User-Agent": "extella-wizard"}))
        with urllib.request.urlopen(rq, timeout=8) as r:
            return json.loads(r.read().decode())

    # канонический путь: фильтр по pipeline-тегу + сорт по загрузкам → топ-модели задачи; фолбэк — поиск по имени
    hf_hit = False
    for hp in ([{"filter": hf_tag}] if hf_tag else []) + ([{"search": q}] if q else []):
        if hf_hit:
            break
        try:
            hp.update({"sort": "downloads", "direction": -1, "limit": 40})
            ms = _get("https://huggingface.co/api/models", hp)
            for m in sorted(ms, key=lambda x: -(x.get("downloads") or 0))[:limit]:
                hf_hit = True
                cands.append({"source": "HuggingFace", "id": str(m.get("id")),
                              "url": "https://huggingface.co/" + str(m.get("id")),
    ...
    try:
        gh = _get("https://api.github.com/search/repositories",
                  {"q": q, "sort": "stars", "per_page": limit},
                  {"Accept": "application/vnd.github+json"})
        for it in (gh.get("items") or [])[:limit]:
            cands.append({"source": "GitHub", "id": str(it.get("full_name")),
...
def _goal_loop(goal, max_iters=3):
    ...
    for it in range(max(1, min(int(max_iters or 3), 6))):
        ...
        if kind == "acquire":
            # ДОБЫЧА СПОСОБНОСТИ: кодоген не тянет → ищем готовую (HF/GitHub). Установка = действие на
            # устройстве → канон предложи-подтверди: находим кандидата и честно эскалируем на гейт человека.
            # деривуем чистый англ. запрос (для GitHub) + канонический HF pipeline-тег (для топ-моделей по тегу)
            dv = _agent_json(ag, "Задача: " + step + "\n\nДля поиска ГОТОВОЙ модели/репозитория верни JSON "
                             '{"query":"<2-4 англ. слова, без кавычек и примеров>","hf_tag":"<канонический HuggingFace '
                             "pipeline-тег из списка или пусто>\"}. Возможные hf_tag: depth-estimation, object-detection, "
                             "image-segmentation, image-classification, text-to-image, image-to-text, "
                             "automatic-speech-recognition, text-to-speech, translation, summarization, "
                             "text-classification, token-classification, question-answering, feature-extraction, "
                             "zero-shot-classification, text-generation. Если ни один не подходит — hf_tag пустой.")
            need = str(dv.get("query") or plan.get("need") or step).strip().splitlines()[0][:60]
            hf_tag = str(dv.get("hf_tag") or "").strip().lower() or None
            cands = _capability_search(need, hf_tag)
            journal.append({"iter": it + 1, "kind": "acquire", "step": step, "candidates": cands})
            if cands:
                top = cands[0]
                q = ("Для шага «" + step[:120] + "» нужна готовая способность (её нельзя написать кодом с нуля). "
                     "Нашёл кандидата: " + top["source"] + " · " + top["id"] + " (" + top["meta"] + "). "
                     "Установка = действие на устройстве — подтвердите, и я добавлю способность в контур.")
            else:
                q = ("Для шага «" + step[:120] + "» нужна внешняя способность (модель/репозиторий), но автоматически "
                     "кандидат не найден. Подскажите источник (ссылка на модель/репо) — и продолжу.")
            return {"ok": True, "status": "need_human", "question": q, "iters": it + 1,
                    "journal": journal, "facts": facts, "memory": memory, "candidates": cands}
        if kind == "build":
            # СЕРДЦЕ КОНТУРА: LLM пишет эксперта под шаг → запускаем в песочнице → реальный результат.
    ...
    def do_POST(self):
        ...
        if self.path == "/x/expert":
            ...
            if not builds or not builds[-1].get("orchestrator"):
                ...
                return
            _tp = _target_preflight(s, mode="manual")   # T2: требования к устройству — ДО прогона
            if not _tp.get("ok"):
