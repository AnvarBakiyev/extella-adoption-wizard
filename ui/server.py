...
BRIDGE_VERSION = "3.53"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
def design_agent():
    ...
    (проверено: Qwen-Визард 9 задач, канонический Qwen Extella 12). ЖЕЛЕЗНОЕ ПРАВИЛО: НИКОГДА Claude — только Qwen.
    Дефолт = свой Qwen-Визард клиента (config.agent_id, базовый qwen3.7-max, есть у каждого клиента).
    ...
    def do_POST(self):
        ...
        if self.path == "/x/expert":
            ...
            if orch == "ci_run_pipeline":
                ...
                res = run_expert("ci_run_pipeline", {"agent_id": "agent_iVWWFbzjmNwxgZNB5chIr", "deliver": "none",
            ...
            if not task:
                ...
                return
            res = run_expert("wz_auto_compose", {"task": task, "api_token": CONFIG.get("auth_token", "")},
                             wait=200, glob=True)
