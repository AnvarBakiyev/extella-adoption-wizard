...
BRIDGE_VERSION = "3.46"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
# Канонический дешёвый платформенный Qwen (доступен всем; его же используют VPS-харвестеры).
# Весь keyless-LLM визарда идёт СЮДА, НЕ на agent_extella_default (=Claude, жёг бы баланс).
CANON_QWEN = "agent_XwZBKvd8dD70jKvW4WrZm"


def qwen_agent():
    """Qwen-агент для keyless-LLM: явный override → канонический Qwen → Визард клиента (гарантированно есть). Никогда Claude."""
    return CONFIG.get("llm_agent_id") or CANON_QWEN or CONFIG.get("agent_id", "")
