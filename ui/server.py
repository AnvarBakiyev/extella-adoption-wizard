...
BRIDGE_VERSION = "4.24"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
        ...
        if kind == "build":
            ...
            if code:
                ...
                if err is None and res is not None:
                    ...
                    journal.append({"iter": it + 1, "kind": "build", "step": step, "result": rs,
                                    "code": code[:600]})
                    continue
                # код не сгенерился/упал/отвергнут песочницей → честно в факты, планировщик перепланирует
            ...
            continue
        # reason: шаг решается рассуждением (LLM даёт результат) → в факты (накопление = память контура)
        result = _agent_text(ag, "Цель: " + goal + "\nВыполни шаг: " + step + "\nНакоплено:\n" + ftext() +
                             "\nВерни КРАТКИЙ конкретный результат этого шага (1-3 предложения), двигающий к цели.")
        facts.append(step + " → " + (result[:280] if result else "(шаг не дал результата)"))
        journal.append({"iter": it + 1, "kind": "reason", "step": step, "result": result[:280]})
