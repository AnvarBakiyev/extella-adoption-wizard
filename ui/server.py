...
BRIDGE_VERSION = "4.58"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
    ...
    def _one(rec):
        ...
        if isinstance(pp, dict) and pp.get("passport_at"):
            try:
                ...
                age = None
        out.append({"target": tgt, "label": rec.get("description") or tgt,
                    "online": pr["online"], "why": pr["why"],
                    "is_host": tgt == HOST_TARGET,
        ...
        if self.path == "/x/expert":
            ...
            if s.get("source"):
                ...
                if _drift.get("breaking"):
                    ...
                    self._send({"status": "error", "code": "source_drift", "message": _msg, "drift": _drift}, 409)
                    return
