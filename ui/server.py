...
BRIDGE_VERSION = "4.06"       # версия моста; /x/health отдаёт её, single-instance по ней решает «свежий/старый»
...
def _store_client_secret(client, connector, value):
    ...
    if api("/api/kv/set", {"key": _secret_kvkey(client, connector), "value": ct, "description": "secret " + connector}).get("status") != "success":
        return False
    _secidx_mark(client, connector, set_at=datetime.now(timezone.utc).isoformat())
    return True
...
def _secidx_entry(v):
    ...
    if isinstance(v, dict):
        return {"set_at": v.get("set_at"), "validated_at": v.get("validated_at"),
                "last_ok": v.get("last_ok"), "last_err": v.get("last_err")}
    return {"set_at": v if isinstance(v, str) else None, "validated_at": None, "last_ok": None, "last_err": None}


def _secidx_mark(client, connector, set_at=None, validated_ok=None, err=None):
    """Обновляет запись индекса: set_at — при записи секрета; validated_at/last_ok/last_err — при тесте/отправке.
    Терпит legacy-строки. НЕ создаёт запись для несуществующего коннектора при чистой валидации."""
    if not connector:
        return
    try:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/x/secret_set":
            ...
            # Помечаем set_at; статус проверки (validated_at/last_ok) проставит тест ниже (#10/#15).
            _secidx_mark(client, connector, set_at=datetime.now(timezone.utc).isoformat())
