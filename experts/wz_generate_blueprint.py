...
{{
  ...
  "crux_layer": {{"layer": "source|capability|knowledge_voice|trust|action|artifact|orchestration", "why": "почему именно этот слой несущий для этой задачи"}},
...
Составь Process Blueprint по правилам."""
    ...
    # -- Несущий слой (crux): всегда присутствует; невалидный → эвристика от источника/архетипа --------
    _CRUX = ("source", "capability", "knowledge_voice", "trust", "action", "artifact", "orchestration")
    cx = bp.get("crux_layer")
    if not isinstance(cx, dict):
        cx = {}
    if str(cx.get("layer", "")) not in _CRUX:
        # эвристика по умолчанию: внешние данные → source; иначе орchestration
        cx = {"layer": ("source" if ds["needs_external"] and ds["obtained_by"] != "file" else "orchestration"),
              "why": cx.get("why", "") if isinstance(cx, dict) else ""}
    cx["why"] = str(cx.get("why", ""))[:400]
    bp["crux_layer"] = cx
